 ...
 * modified 2016-06-11: filename changed for WDG project
 ...
 * 
 * FTP command overview: https://en.wikipedia.org/wiki/List_of_FTP_commands
 */ 
...
String fileName = "AF104net.txt";
String path = "/AF104net.txt";
...
byte doFTP(boolean upload) {
  ...
  if (upload) {
    if (debug) Serial.println("Send CWD path");
    client.print("CWD ");
    client.println("WDG");
    if (debug) Serial.println("Send PWD");
    client.println("PWD");
...
void setup() {
  ...
  if (!fh) {
    ...
    Serial.println(F(": SPIFFS open fail for READ"));
    signalError();
  } else fh.close();
  
  fh = SPIFFS.open(path, "a+");
  if (!fh) {
    Serial.print(__func__);
